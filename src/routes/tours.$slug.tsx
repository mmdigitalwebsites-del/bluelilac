...
import { ArrowLeft, ArrowRight, Clock, Users, MapPin, Star, Mail, Phone, Check, X, CalendarDays, BedDouble } from "lucide-react";
...
function buildItinerary(explicit: ItineraryDay[] | undefined, highlights: string, durationDays: number): ItineraryDay[] {
  ...
  stops.forEach((stop, idx) => {
    ...
    days.push({
      ...
      bullets: isArrival
        ? [`Arrive in ${stop} and meet your Bluelilac driver-guide.`, "Transfer to your lodge, briefing on the days ahead.", "Dinner and overnight."]
        : [`Game drives and guided activities around ${stop}.`, "Picnic or lodge lunch, with afternoon wildlife viewing.", "Return to lodge for dinner and overnight."],
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <section className="relative flex h-[60vh] min-h-[460px] w-full items-end overflow-hidden">
        <img src={tour.img} alt={tour.title} className="absolute inset-0 h-full w-full object-cover" />
        ...
        <div className="relative z-10 mx-auto w-full max-w-7xl px-6 pb-14 md:px-10">
          <Link to="/tours" className="inline-flex items-center gap-2 text-sm text-white/80 hover:text-white">
          ...
          <div className="mt-6 flex flex-wrap items-center gap-5 text-sm text-white/90">
            ...
            <span className="inline-flex items-center gap-2"><Users className="h-4 w-4" /> {tour.group} guests</span>
            <span className="inline-flex items-center gap-2"><MapPin className="h-4 w-4" /> {tour.highlights}</span>
            <span className="inline-flex items-center gap-1 text-primary">
              {Array.from({ length: 5 }).map((_, i) => (
                <Star key={i} className={`h-4 w-4 ${i < tour.rating ? "fill-current" : "opacity-30"}`} />
              ))}
              <span className="ml-1 text-white/90">{tour.rating}.0 ({tour.reviews})</span>
            </span>
          </div>
        </div>
      </section>
            ...
            <div className="space-y-12">
              ...
              <div>
                ...
                <ul className="mt-6 grid gap-3 sm:grid-cols-2">
                  {tour.highlights.split("·").map((h: string) => h.trim()).filter(Boolean).map((h: string) => (
                    <li key={h} className="flex items-start gap-3 rounded-2xl border border-border bg-card p-4 text-sm text-black">
              ...
              <div>
                ...
                <ol className="mt-8 relative space-y-6 border-l border-border pl-6 md:pl-8">
                  {buildItinerary(tour.itinerary, tour.highlights, tour.durationDays).map((d, i) => (
              ...
              <div>
                ...
                <p className="mt-4 text-sm leading-relaxed text-black md:text-base">
                  You can cancel up to 24 hours in advance of the experience for a full refund. For tailor-made private trips, our team will share the specific terms with your quote.
                </p>
              </div>
            </div>
            ...
            <aside className="lg:sticky lg:top-24 lg:self-start">
              <div className="rounded-3xl border border-border bg-card p-6 shadow-sm">
                <div className="flex items-baseline gap-1">
                  ...
                  <span className="font-display text-4xl text-primary">${tour.price.toLocaleString()}</span>
                ...
                <div className="mt-5 space-y-3 border-t border-border pt-5 text-sm text-black">
                  <div className="flex items-center justify-between"><span className="text-black/60">Duration</span><span>{tour.duration}</span></div>
                  <div className="flex items-center justify-between"><span className="text-black/60">Group size</span><span>{tour.group} guests</span></div>
                  <div className="flex items-center justify-between"><span className="text-black/60">Tour type</span><span>{tour.type}</span></div>
                  <div className="flex items-center justify-between"><span className="text-black/60">Language</span><span>English</span></div>
                </div>
                <a
            ...
            {related.map((r) => (
              <article key={r.slug} className="group overflow-hidden rounded-3xl bg-card transition hover:-translate-y-1">
                <Link to="/tours/$slug" params={{ slug: r.slug }} className="block">
                  ...
                  <div className="p-5">
                    <h3 className="font-display text-lg leading-snug group-hover:text-primary">{r.title}</h3>
                    <div className="mt-4 flex items-center justify-between border-t border-border pt-4 text-sm">
                      <span className="text-black">{r.duration}</span>
                      <span className="font-display text-lg text-primary">${r.price.toLocaleString()}</span>
                    </div>
